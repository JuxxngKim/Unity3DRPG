...
namespace UnityEngine.UI
{
    ...
    public abstract class RecyclableScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        //==========Recyclable ScrollRect==========
        [Tooltip("Prefab Source")]
        public GameObject prefabSource = null;
        ...
        [HideInInspector]
        [NonSerialized]
        public IRecyclableScrollDataSource dataSource = null;
        ...
        protected enum RecyclableScrollRectDirection
        {
            Vertical,
            Horizontal,
        }
        protected RecyclableScrollRectDirection direction = RecyclableScrollRectDirection.Horizontal;
        ...
        protected RecyclableScrollRect()
        {
            flexibleWidth = -1;
        }
        ...
        protected override void Awake()
        {
            ...
            {
                float value = (reverseDirection ^ (direction == RecyclableScrollRectDirection.Horizontal)) ? 0 : 1;
        ...
        public void ClearCells()
        {
            if (Application.isPlaying)
            {
                ...
                {
                    GameObjectCache.Delete(content.GetChild(i));
            ...
            while (needMoving)
            {
                ...
                if (!m_Dragging)
                {
                    ...
                    else
                    {
                        ...
                        var offset = 0.0f;
                        if (direction == RecyclableScrollRectDirection.Vertical)
                        ...
                        if (totalCount >= 0)
                        {
                            if (offset > 0 && itemTypeEnd == totalCount && !reverseDirection)
                            {
                                ...
                                // reach bottom
                                if ((direction == RecyclableScrollRectDirection.Vertical && m_ItemBounds.min.y > m_ViewBounds.min.y) ||
                                    (direction == RecyclableScrollRectDirection.Horizontal && m_ItemBounds.max.x < m_ViewBounds.max.x))
                                {
                                    needMoving = false;
                                    break;
                                }
                            ...
                            else if (offset < 0 && itemTypeStart == 0 && reverseDirection)
                            {
                                ...
                                if ((direction == RecyclableScrollRectDirection.Vertical && m_ItemBounds.max.y < m_ViewBounds.max.y) ||
                                    (direction == RecyclableScrollRectDirection.Horizontal && m_ItemBounds.min.x > m_ViewBounds.min.x))
                                {
                                    needMoving = false;
                                    break;
                                }
        ...
        protected void ClearTempPool()
        {
            ...
            {
                deletedItemTypeStart--;
                GameObjectCache.Delete(content.GetChild(0));
                //prefabSource.ReturnObject(content.GetChild(0));
            }
            while (deletedItemTypeEnd > 0)
            {
                deletedItemTypeEnd--;
                GameObjectCache.Delete(content.GetChild(content.childCount - 1));
